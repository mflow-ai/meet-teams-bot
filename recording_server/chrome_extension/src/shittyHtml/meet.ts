...
export async function removeInitialShityHtml(mode: RecordingMode) {
    ...
    try {
        document
            ...
            .forEach((elem) => ((elem as HTMLElement).style.display = 'none'))
    } catch (e) {}
    try {
...
function findMainVideo(doc: Document): HTMLVideoElement | null {
    ...
    const videoCandidates = Array.from(
        doc.querySelectorAll('video[playsinline]'),
    )
...
function hideInterfaceElements(doc: Document): void {
    ...
    const bottomElements = Array.from(doc.querySelectorAll('div')).filter(
        (div) => {
