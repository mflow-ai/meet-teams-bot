...
function calculateExtensionId(publicKey: string): string {
    ...
    const hash = crypto.createHash('sha256').update(keyBuffer as any).digest()
...
function main() {
    const manifestPath = path.join(__dirname, '..', '..', 'chrome_extension', 'dist', 'manifest.json')
    ...
    try {
        ...
        if (fs.existsSync(browserTsPath)) {
            ...
            const idMatch = browserContent.match(/const EXTENSION_ID = '([^']+)'/)
            ...
            if (idMatch) {
                ...
                if (calculatedId === hardcodedId) {
                    ...
                    console.log('   Update browser.ts with the calculated ID above')
                }
            }
        }
